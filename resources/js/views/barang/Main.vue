<template>
    <div class="flex-auto w-full h-full box-border p-6 overflow-hidden flex flex-col relative">
        <div class="mb-5 w-full box-border flex items-center justify-between">
            <div class="font-bold text-gray-600 text-lg 2xl:text-2xl">Dataset</div>
        ...
        <div class="w-full box-border flex justify-between items-center mb-5">
            ...
            <div class="w-1/2 relative flex items-center">
                ...
                <input v-model="params.cari" @change="getDataset" type="text" class="pl-10 w-full bg-gray-200 px-4 py-3 outline-none text-xs rounded-lg focus:shadow-lg" placeholder="cari bulan lalu enter ...">
            </div>

            <div class="flex items-center justify-center space-x-5">
                <form @submit.prevent="uploadFile()" class="flex">
                    <input class="text-xs block w-full px-2 py-px transition duration-100 ease-in-out border rounded shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none focus:ring-opacity-50 disabled:opacity-50 disabled:cursor-not-allowed text-xs text-black placeholder-gray-400 bg-white border-gray-300 focus:border-blue-500 " type="file" ref="uploadFIle">
                    <button type="submit" class="text-xs rounded-lg border border-gray-200 bg-gray-200 text-gray-500 font-semibold px-4 py-2 flex items-center hover:text-blue-400">
                ...
                <button @click.prevent="$refs.modalTambah.show()" class="text-xs rounded-lg border border-gray-200 bg-gray-200 text-gray-500 font-semibold px-4 py-2 flex items-center hover:text-blue-400">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 13h6m-3-3v6m5 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    <span>Tambah</span>
        ...
        <div class="flex flex-col justify-between w-full h-full space-x-4 overflow-auto bg-white">
                ...
                <table class="w-full table-auto text-xs 2xl:text-sm mb-14">
                    <thead class="font-semibold text-gray-500">
                        <tr class="relative border-b shadow-md sticky top-0">
                            ...
                            <th class="p-3 whitespace-nowrap capitalize text-left sticky top-0 bg-white">Minggu Ke</th>
                            <th class="p-3 whitespace-nowrap capitalize text-left sticky top-0 bg-white">Bulan</th>
                            <th class="p-3 whitespace-nowrap capitalize text-left sticky top-0 bg-white">Tahun</th>
                    ...
                    <tbody v-else class="bg-white">
                        <template v-if="dataset.length <= 0">
                            <tr>
                                <td colspan="10" class="px-3 py-10 text-center italic font-semibold">Tidak ada Data penjualan.</td>
                            </tr>
                        </template>

                        <template v-else>
                            <tr v-for="(item, i) in dataset" :key="i">
                                <td width="2%" class="px-3 py-4 border-b border-gray-200 whitespace-nowrap capitalize font-semibold">{{ (currentPage - 1) * perPage + 1 + i }}.</td>
                                <td class="px-3 py-4 border-b border-gray-200 whitespace-nowrap capitalize">{{ item.minggu_ke }}</td>
                                <td class="px-3 py-4 border-b border-gray-200 whitespace-nowrap capitalize">{{ item.bulan }}</td>
                                <td class="px-3 py-4 border-b border-gray-200 whitespace-nowrap capitalize">{{ item.tahun }}</td>
                                ...
                                <td class="px-3 py-4 border-b border-gray-200 whitespace-nowrap capitalize">{{ item.produksi }}</td>
                                <td width="25px" class="px-3 py-4 border-b border-gray-200 whitespace-nowrap capitalize">
                                    <button @click.prevent="openModalEdit(item)" class="text-sx text-gray-400 hover:text-blue-400">
            ...
            <div class="absolute bottom-0 right-0 left-0 box-border overflow-hidden p-6 text-sm pl-3">
                <div v-if="lastPage > 1" class="w-full box-border flex items-center justify-between bg-white p-4 z-50 border-t border-gray-200">
        ...
        <modal ref="modalHapus">
            <div slot="modal-body" class="w-full text-xs px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div>
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            <button @click.prevent="deleteDataset()" type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">
                                Ya, Hapus
                            </button>
                            <button @click.prevent="closeModalHapus()" type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                                Batal
                            </button>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        async getDataset(){
            try{
                let { data } = await axios.get(`dataset`, {params: this.params})
        ...
        async editData(){
            try{
                let res = await axios.post(`dataset/${this.payload.id}/update`, this.payload)
        ...
        async deleteDataset(){
            try{
                let res = await axios.delete(`dataset/${this.payload.id}/delete`)
